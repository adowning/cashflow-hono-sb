...
export const seedGames = async () => {
	const _games = await fetch("https://configs.cashflowcasino.com/house/games_small.json");
...
async function setupOperators() {
	try {
		...
		if (!existingOperator) {
			...
			const _products = await fetch("https://configs.cashflowcasino.com/house/products.json");
		...
		console.error("✗ BonusData, Settings, Games and Operator setup failed:", error);
		return false;
	}
}
...
if (!process.env.ADMIN_PASSWORD) {
	console.warn("ADMIN_PASSWORD environment variable is not set. Using a randomly generated password for admin.");
}
	...
	try {
		...
		if (result) {
			...
			if (userRecord) {
				...
				await db.update(userTable).set({ banned: false }).where(eq(userTable.id, userRecord.id));
			...
			try {
				...
				if (result) {
					...
					if (userRecord) {
						...
						await db.update(userTable).set({ createdAt }).where(eq(userTable.id, userRecord.id));
