...
import {
  INDEX_OUT_OF_RANGE_ERROR_LABEL,
  INVALID_COUNT_ERROR_LABEL,
  ITEM_NOT_EXISTS_ERROR_LABEL,
  NO_ARRAY_ERROR_LABEL,
} from "./errors";
import { getDatabase, getStatements } from "./datacache";
...
export class BunSqliteKeyValue {
  ...
  constructor(filename?: string, options?: Options) {
    // Parse options
    const { ttlMs, maxExpiringItemsInDb, ...otherOptions } = options ?? {};
    this.ttlMs = ttlMs;
  ...
  getCountValid(deleteExpired?: boolean): number {
    if (deleteExpired === true) {
      ...
      return (
        this.statements.countValid.get({ now: Date.now() }) as { count: number }
      ).count;
    }
  }
   ...
   */
  setItems<T = any>(
    items: { key: Key | undefined; value: T; ttlMs?: TtlMs }[]
  ) {
    this.db.transaction(() => {
  ...
  getItemsAsObject<T = any>(
    startsWithOrKeys?: string | string[]
  ): { [key: Key]: T | undefined } | undefined {
  ...
  deleteOldExpiringItems(maxExpiringItemsInDb?: number) {
    let maxExpiringItems: number | undefined =
      maxExpiringItemsInDb ?? this.maxExpiringItemsInDb;
  ...
  getRandomKey(): string | undefined {
    return (
      this.statements.getRandomKey.get({ now: Date.now() })?.key ?? undefined
    );
  }
  ...
  hmGet<T = any>(
    key: Key,
    fields?: string[]
  ): { [field: Field]: T | undefined } | undefined {
    const map = this.get<Map<string, T>>(key);
    if (map === undefined) return;
  ...
  hIncr(key: Key, field: Field, incrBy: number = 1, ttlMs?: TtlMs): number {
    ...
    return this.db
      .transaction(() => {
        const map =
          this.get<Map<string, number>>(key) ?? new Map<string, number>();
        let newValue: number;
        try {
      ...
      .transaction(() => {
        ...
        try {
          ...
          if (error.toString().includes("TypeError")) {
            throw new Error(
              NO_ARRAY_ERROR_LABEL +
                ` Value at "${key.substring(-80)}" is not an array.`
            );
          }
          throw error;
        }
      ...
      .transaction(() => {
        ...
        try {
          if (count === undefined) {
            ...
            throw new Error(
              INVALID_COUNT_ERROR_LABEL + " `count` must be greater then 0."
            );
          }
        } catch (error: any) {
          if (error.toString().includes("TypeError")) {
    ...
    return this.db
      .transaction(() => {
        ...
        if (array === undefined) {
          throw new Error(
            ITEM_NOT_EXISTS_ERROR_LABEL +
              ` Key "${key.substring(-80)}" not found.`
          );
        }
        if (Array.isArray(array) === false) {
          throw new Error(
        ...
        if (index >= len || index < len * -1) {
          throw new Error(
            INDEX_OUT_OF_RANGE_ERROR_LABEL + ` Array length: ${len}`
          );
        }
        if (index < 0) {
