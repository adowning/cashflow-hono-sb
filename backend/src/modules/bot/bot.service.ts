...
import { gameSessionTable, gameTable, sessionTable, userTable } from "../../core/database/schema";
import { v4 as uuidv4 } from "uuid";
import { processBet, type BetOutcome } from "../gameplay/orchestrators/bet.orchestrator";
...
export class BotService {
	...
	constructor(config: Partial<BotConfig> = {}, dependencies?: Partial<BotServiceDependencies>) {
	...
	async initialize(): Promise<boolean> {
		...
		if (!botPassword) {
			throw new BotInitializationError("BOT_PASSWORD environment variable must be set");
		}

		if (!botEmail) {
			throw new BotInitializationError("BOT_EMAIL environment variable must be set");
		}
	...
	async initializeWithUser(userId: string | null, email: string, password: string): Promise<boolean> {
		try {
			if (this.config.betInterval < 1000) {
				throw new BotInitializationError("Bet interval must be at least 1000ms");
			}
	 ...
	 */
	private async findOrCreateBotUser(username: string, email: string, password: string): Promise<UserSelect> {
		...
		if (signUpResult.error || !signUpResult.data.user) {
			throw new BotInitializationError(`Failed to create bot user: ${signUpResult.error?.message}`);
		}
	...
	private async initializeGameSession(): Promise<void> {
		...
		const existingSession = await this.dependencies.database.query.gameSessionTable.findFirst({
			where: and(eq(gameSessionTable.userId, this.userId), eq(gameSessionTable.status, "ACTIVE")),
		});
	...
	private async authenticateBotUser(email: string, password: string): Promise<void> {
		...
		if (signInResult.error || !signInResult.data.session) {
			throw new BotAuthenticationError(`Failed to sign in bot user: ${signInResult.error?.message}`);
		}
		...
		if (error) {
			throw new BotAuthenticationError(`Failed to get session: ${error.message}`);
		}
		...
		if (session && session.access_token) {
			try {
				...
				throw new BotAuthenticationError("Failed to create/update session record", e);
			}
		}
	}
	...
	private async placeBet(): Promise<BetOperationResult> {
		...
		try {
			...
			const gameLimits = await botStrategy.getGameLimits(this.gameId, this.config);
			...
			if (!balanceResult.success) {
				throw new BotOperationError("Failed to ensure sufficient balance", balanceResult.error);
			}
			...
			const wagerAmount = botStrategy.getWager(this.config, game, finalBalance, gameLimits);
			...
			if (this.lastBetTime) {
				const timeSinceLastBet = (currentTime.getTime() - this.lastBetTime.getTime()) / 1000 / 60; // minutes
				this.totalMinutesPlayed += Math.max(0, timeSinceLastBet);
			} else if (this.sessionStartTime) {
				const timeSinceSessionStart = (currentTime.getTime() - this.sessionStartTime.getTime()) / 1000 / 60;
	...
	private isAuthenticationError(error: unknown): boolean {
		...
		return (
			message.includes("session expired") || message.includes("not authenticated") || message.includes("unauthorized")
		);
	}
		...
		try {
			...
			if (this.gameSessionId) {
				await this.dependencies.database
					...
					.where(and(eq(gameSessionTable.id, this.gameSessionId), eq(gameSessionTable.userId, this.userId)));
				this.results = [];
			}
	...
	getMetrics(): {
		...
		const successRate = totalBets > 0 ? (this.results.filter((r) => r.winAmount > 0).length / totalBets) * 100 : 0;
		const totalWagered = this.results.reduce((sum, r) => sum + r.wagerAmount, 0);
 ...
 */
export async function startManufacturedGameplay(config: Partial<BotConfig> = {}): Promise<void> {
	try {
