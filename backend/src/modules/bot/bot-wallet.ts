...
export async function ensureSufficientBalance(
	...
	if (lastDepositAttemptTime && Date.now() - lastDepositAttemptTime.getTime() > DEPOSIT_ATTEMPT_COOLDOWN_MS) {
	...
	try {
		...
		const newTotalBalance = updatedBalance.realBalance + updatedBalance.bonusBalance;
