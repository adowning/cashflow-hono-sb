...
export class BotManager {
	...
	constructor(config: BotManagerConfig) {
		...
		if (this.config.botCount < 1 || this.config.botCount > 20) {
			throw new BotManagerInitializationError("Bot count must be between 1 and 20");
		}
	}
	...
	private async fetchBotUsers(count: number): Promise<Array<{ id: string; username: string; authEmail: string }>> {
        ...
        `,
			...
			throw new BotManagerInitializationError("Failed to fetch bot users from database", error);
		}
	}
	...
	async initialize(): Promise<void> {
		try {
			...
			const successfulBots = Array.from(this.botInstances.values()).filter((instance) => instance.status !== "error");

			if (successfulBots.length === 0) {
				throw new BotManagerInitializationError("Failed to initialize any bot instances");
			}

			if (successfulBots.length < this.config.botCount) {
				console.warn(`Only ${successfulBots.length} out of ${this.config.botCount} bots were successfully initialized`);
			}
			...
			throw new BotManagerInitializationError("Failed to initialize BotManager", error);
		}
	}
	...
	async start(): Promise<void> {
		...
		if (this.botInstances.size === 0) {
			throw new BotManagerOperationError("No bot instances available. Call initialize() first.");
		}
	...
	private getRunningBotsCount(): number {
		return Array.from(this.botInstances.values()).filter((instance) => instance.status === "running").length;
	}

	private getErrorBotsCount(): number {
		return Array.from(this.botInstances.values()).filter((instance) => instance.status === "error").length;
	}

	private getStoppedBotsCount(): number {
		return Array.from(this.botInstances.values()).filter((instance) => instance.status === "stopped").length;
	}
 ...
 */
export async function startManufacturedGameplay(config: Partial<BotManagerConfig> = {}): Promise<BotManager> {
	try {
