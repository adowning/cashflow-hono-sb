 ...
 */
export async function getGameLimits(gameId: string, botConfig: BotConfig): Promise<{ minBet: number; maxBet: number }> {
	if (!gameId) {
	...
	try {
		...
		const minBet = game.minBet ?? botConfig.minWager ?? systemLimits?.minBetAmount ?? 100;
		const maxBet = game.maxBet ?? botConfig.maxWager ?? systemLimits?.maxBetAmount ?? 100000;
...
function getCalculatedWager(
	...
	const effectiveMaxGameBet = Math.min(gameLimits.maxBet, balance.totalBalance);

	// Respect the *bot's* configured max wager
	const effectiveMaxBotBet = Math.min(effectiveMaxGameBet, botConfig.maxWager);

	// Add randomness (e.g., bet between 70% and 100% of the max allowed)
	const randomizedMaxWager = effectiveMaxBotBet * (0.7 + Math.random() * 0.3);
