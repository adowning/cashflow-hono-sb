 ...
 */
export async function onDepositCompleted(
  payload: any
  ...
  try {
    ...
    if (xpGained > 0 && depositId) {
      try {
        ...
        console.error(
          `Failed to enrich transaction log ${depositId} with VIP points:`,
          txError
        );
...
async function checkFirstTimeDeposit(userId: string): Promise<boolean> {
  const depositCount = await db
    ...
    .from(depositTable)
    .where(
      and(eq(depositTable.userId, userId), eq(depositTable.status, "COMPLETED"))
    );
